#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""HAS fill-only extractor v2.4 - Fixed version
Combines the working logic from v2.3d with improved structure.
"""
...
def load_config(path: Optional[Path]) -> dict:
    ...
    try:
        ...
        raise ConfigValidationError(f"unable to read config {resolved}: {exc}") from exc
...
def process(pdf_root: Path, excel_in: Path, excel_out: Path, config_path: Optional[Path]) -> None:
    ...
    logger.info("found %d unique CTs in PDF filenames", len(ct_to_pdfs))
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="HAS fill-only extractor v2.4 (fixed)")
    parser.add_argument("pdf_root", type=Path, help="Root folder containing PDFs")
    parser.add_argument("excel_in", type=Path, help="Input Excel file")
    parser.add_argument("excel_out", type=Path, help="Output Excel file")
    parser.add_argument(
        "--config",
        type=Path,
        default=None,
        help="Optional YAML config file (embedded defaults used if omitted)",
    )
